...
def _build_s3(config: dict, arn: str, region: str, account_id: str, tags: dict) -> dict:
    ...
    acl_grants = config.get("acl_grants") or []
    public_acl = any(
        g.get("grantee", "") in ("http://acs.amazonaws.com/groups/global/AllUsers", "http://acs.amazonaws.com/groups/global/AuthenticatedUsers")
        for g in acl_grants
    )
