...
def run_inventory_pull(tenant_id: int, provider_id: int, run) -> dict:
    ...
    for batch in stagger_accounts(accounts, batch_size=10, delay_seconds=180):
        for account in batch:
            _process_account(account, role_name, neo4j_driver, stats, tenant_id, provider, run)

    return stats


def _process_account(account, role_name, neo4j_driver, stats, tenant_id, provider, run):
    ...
    if settings.enable_s3_fallback_discovery:
    ...
    if settings.enable_aws_config_drift:
        cfg_region = settings.aws_config_region or AGGREGATOR_REGION
        config_changed_arns = collect_changed_arns_from_config(
            account_id=account_id,
            role_name=role_name,
            region=cfg_region,
            lookback_minutes_default=settings.aws_config_initial_lookback_minutes,
        )
    ...
    for arn in delta_arns:
        ...
        try:
            ...
            with SessionLocal() as db:
                merged_tags = {**(resource_meta.get("tags") or {}), **(config.get("tags") or {})}
    ...
    for arn in delta["deleted"]:
        ...
        with SessionLocal() as db:
            repo.mark_resource_config_deleted(db, account_id, arn)
        new_hashes.pop(arn, None)

    save_hashes(account_id, new_hashes)

    # ----- Re-evaluate stored configs not in this run's delta (rule updates, consolidated rules) -----
    with SessionLocal() as db:
        stored_configs = repo.list_resource_configs_for_account(db, account_id)
    re_eval_count = 0
    for rc in stored_configs:
        ...
        try:
            ...
            config = rc.config
            merged_tags = {**(rc.tags or {}), **((config or {}).get("tags") or {})}
            fetcher, effective_type = _get_fetcher(rc.resource_type, rc.arn)
            if fetcher:
                session_region = rc.region if rc.region and rc.region != "global" else "us-east-1"
                regional_session = get_session(account_id, role_name, session_region)
                config = fetcher(regional_session, rc.arn, rc.region)
                merged_tags = {**merged_tags, **(config.get("tags") or {})}
                with SessionLocal() as db:
                    repo.upsert_resource_config(
                        db,
    ...
    if re_eval_count:
        logger.info("Re-evaluated %s stored resources", re_eval_count)
